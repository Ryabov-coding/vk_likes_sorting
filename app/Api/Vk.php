...
class Vk {
    ...
    public function __construct(Meta $meta) {
        $this->meta = $meta;
        $this->token = $this->meta->getConfigParam('vk_api_token');
    }
    ...
    public function idByName($name) {
        ...
        if (!function_exists('curl_init')) {
            throw new Exception('Curl module doesn\'t exist on server!');
        }
        $ch = curl_init($url);
     ...
     * @return mixed
     */
    public function executeGetPostsApiScript($id, $offset) {
        $code = '
        var iters = 25;
        var lists = [];
        var params = {
